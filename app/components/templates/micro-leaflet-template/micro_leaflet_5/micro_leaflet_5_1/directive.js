...
define(["ionic","qrCode"], function () {
    angular.module("MicroLeaflet5_1.directives", [])
        .directive("microLeaflet5by1", [
            "$window", "$timeout", "$rootScope", "microLeaflet5_1Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope, microLeaflet5_1Service, uploadImgService, promptBarService, maskService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-leaflet-template/micro_leaflet_5/micro_leaflet_5_1/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(microLeaflet5_1Service.model);
                                scope.templateModel = scope.sectionModel.templateModel;
                            }
                            //如果是预览，给下载功能添加打印该页的任务
                            if( scope.status=="preview"){
                                microLeaflet5_1Service.setDownloadConfig(angular.copy(scope.templateModel));
                            }
                        ...
                        scope.updateImg = function (imgIndex) {
                            ...
                            uploadImgService.upLoadImg(microLeaflet5_1Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]), 1, scope.upLoadFinish);
                        };
