...
define(["ionic"], function () {
    angular.module("MicroLeaflet3_3.directives", [])
        .directive("microLeaflet3by3", [
            "$window", "$timeout","$rootScope", "microLeaflet3_3Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope,microLeaflet3_3Service, uploadImgService, promptBarService, maskService,commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-leaflet-template/micro_leaflet_3/micro_leaflet_3_3/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(microLeaflet3_3Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            microLeaflet3_3Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = microLeaflet3_3Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
                        ...
                        scope.updateImg = function (imgIndex) {
                            ...
                            uploadImgService.upLoadImg(microLeaflet3_3Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]), 1, scope.upLoadFinish);
                        };
