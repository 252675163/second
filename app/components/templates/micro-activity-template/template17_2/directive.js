...
define(["ionic"], function () {
    angular.module("Template17_2.directives", [])
        .directive("template17by2", [
            "$window", "$timeout","$rootScope", "template17_2Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope,template17_2Service, uploadImgService, promptBarService, maskService,commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template17_2/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template17_2Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template17_2Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template17_2Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
                        ...
                        scope.updateImg = function (imgIndex) {
                            ...
                            uploadImgService.upLoadImg(template17_2Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]), 1, scope.upLoadFinish);
                        };
