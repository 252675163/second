...
define(["ionic"], function () {
    angular.module("Template10_6.directives", [])
        .directive("template10by6", [
            "$window", "$timeout", "$rootScope", "template10_6Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope, template10_6Service, uploadImgService, promptBarService, maskService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template10_6/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template10_6Service.model);
                        ...
                        //图片比例
                        scope.imgAspectRatio = [10.317,0.95,0.96];
                        ...
                        scope.updateImg = function(imgIndex){
                            ...
                            uploadImgService.upLoadImg( template10_6Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]),1,scope.upLoadFinish);
                        };
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template10_6Service.saveInfo(date, scope.userType).then(function (result) {
                                scope.isSubmit = false;
                                if (result.data.status == 1) {
                                    if (scope.userType == "old") {
                                        //分享遮罩
                                        maskService.showMask("", 0, true, 2).then(function() {
                                            //$rootScope.$state.go("activity.oldandnewview", { oldUser: result.data.data });
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template10_6Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
