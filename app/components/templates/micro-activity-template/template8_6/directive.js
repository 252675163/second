...
define(["ionic"], function () {
    angular.module("Template8_6.directives", [])
        .directive("template8by6", [
            "$window", "$timeout", "$rootScope", "template8_6Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope, template8_6Service, uploadImgService, promptBarService, maskService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template8_6/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template8_6Service.model);
                        ...
                        //图片比例
                        scope.imgAspectRatio = [1.185,0.68,0.68];
                        ...
                        scope.updateImg = function(imgIndex){
                            ...
                            uploadImgService.upLoadImg( template8_6Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]),1,scope.upLoadFinish);
                        };
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template8_6Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template8_6Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
