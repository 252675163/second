...
define(["ionic"], function() {
    angular.module("Template5_6.directives", [])
        .directive("template5by6", [
                "$window", "$timeout", "$rootScope", "template5_6Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function($window, $timeout, $rootScope, template5_6Service, uploadImgService, promptBarService, maskService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template5_6/template.html",
                        link: function(scope, iElement, iAttr) {
                            ...
                            function init() {
                                if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                    scope.sectionModel.templateModel = angular.copy(template5_6Service.model);
                                ...
                                iElement[0].getElementsByClassName("bgPan")[0].style.backgroundSize = "cover";
                            ...
                            scope.submitInfo = function() {
                                ...
                                scope.isSubmit = true;
                                template5_6Service.saveInfo(date, scope.userType).then(function(result) {
                            ...
                            //表单校验
                            function validForm() {
                                var validState = template5_6Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                                if (validState == 1) {
