...
define(["ionic"], function() {
    angular.module("Template13_4.directives", [])
        .directive("template13by4", [
                "$window", "$timeout", "$rootScope", "template13_4Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function($window, $timeout, $rootScope, template13_4Service, uploadImgService, maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template13_4/template.html",
                        link: function(scope, iElement, iAttr) {
                            ...
                            scope.renderListByInit = function() {
                                if (scope.isView && ($rootScope.$state.params.step != "2")) {
                                    ...
                                    template13_4Service.getGrassUserInfosByPage(scope.userId, scope.userType, 1).success(function(result) {
                                        if (result.status == 1) {
...
//                                            scope.page = result.data.page;
                            ...
                            scope.loadMore = function() {
                                template13_4Service.getGrassUserInfosByPage(scope.userId, scope.userType, scope.page.currentIndex + 1).success(function(result) {
                            ...
                            template13_4Service.setRenderCallback(scope.renderListByInit);
                            init();
                        }
