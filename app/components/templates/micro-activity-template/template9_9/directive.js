...
define(["ionic"], function () {
    angular.module("Template9_9.directives", [])
        .directive("template9by9", [
            "$window", "$timeout", "template9_9Service", "$rootScope", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, template9_9Service, $rootScope, uploadImgService, promptBarService, maskService,commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template9_9/template.html",
                    link: function (scope, iElement, iAttr) {
                        ...
                        function init() {
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template9_9Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template9_9Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template9_9Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
