...
define(["ionic"], function () {
    angular.module("Template12_9.directives", [])
        .directive("template12by9", [
            "$window", "$timeout", "$rootScope", "template12_9Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function ($window, $timeout, $rootScope, template12_9Service, uploadImgService, maskService, promptBarService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template12_9/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template12_9Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            //保存数据
                            var nameArr = [scope.templateModel.formOtherInfo1, scope.templateModel.formOtherInfo2];
                            var valueArr = [scope.userInfo.age, scope.userInfo.favorClass];
                            ...
                            var date = {
                                ...
                                Content: JSON.stringify(template12_9Service.getContent(nameArr,valueArr)),
                            };

                            scope.isSubmit = true;
                            template12_9Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template12_9Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
                        ...
                        scope.updateImg = function(imgIndex){
                            ...
                            uploadImgService.upLoadImg( template12_9Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]),1,scope.upLoadFinish);
                        };
