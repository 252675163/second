...
define(["ionic","qrCode"], function () {
    angular.module("Template25_1.directives", [])
        .directive("template25by1", [
            "$window", "$timeout","$rootScope", "template25_1Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope,template25_1Service, uploadImgService, promptBarService, maskService,commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template25_1/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template25_1Service.model);
                                scope.templateModel = scope.sectionModel.templateModel;
                            }
                            //如果是预览，给下载功能添加打印该页的任务
                            if( scope.status=="preview"){
                                template25_1Service.setDownloadConfig(angular.copy(scope.templateModel));
                            }
                        ...
                        scope.updateImg = function (imgIndex) {
                            ...
                            uploadImgService.upLoadImg(template25_1Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]), 1, scope.upLoadFinish);
                        };
