...
define(["ionic", "components/templates/micro-activity-template/template24_1/service"], function () {
    angular.module("Template24_1Step2.directives", ["Template24_1.Service"])
        .directive("template24by1Step2", [
                "$window", "$timeout", "$rootScope", "template24_1Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", "userTermsService", function ($window, $timeout, $rootScope, template24_1Service, uploadImgService, maskService, promptBarService, commonNetService, userTermsService) {
                    return {
                        restrict: "EA",
                        scope: {
                            status: "=",
                        ...
                        templateUrl: "components/templates/micro-activity-template/template24_1/templates_steps/template_step2.html",
                        link: function (scope, iElement, iAttr) {
                            scope.getImageDoMain = function (url) {
                                return window.cdnDoMain + scope.imageFileName + url;
                            }

                            var defaultName = "";
                            var defaultHeadImgUrl = scope.getImageDoMain('/default_avatar.png');
                            //表单页用到的数据
                            scope.userInfo = {
                                name: "",
                                phone: "",
                            ...
                            function init() {
                                if (scope.status != "view") {
                                    ...
                                    template24_1Service.getWeixinUserInfo().success(function (result) {
                                        if (result.status == 1) {
                                            scope.userInfo = {
                                                name:  defaultName,
                                                phone: "",
                            ...
                            scope.submitInfo = function () {
                                ...
                                scope.isSubmit = true;
                                template24_1Service.saveInfo(date).then(function (result) {
                                    ...
                                    if (result.data.status == 1) {
                                        ...
                                        if (config) {
                                            ...
                                            config.imgUrl = scope.shareConfigByVisitor[0].imgUrl || scope.userInfo.headImg || config.imgUrl;
                                        }




                                        // 跳转至个人主页 页面按钮显示逻辑改变（只显示给自己捕鱼）
                                        //直接跳转至分享页面
                                        $rootScope.$state.go("activity.oldandnewview", { oldUser: result.data.data, step: 1 });
                            ...
                            //表单校验
                            function validForm() {
                                var validState = template24_1Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                                if (validState == 1) {
                            ...
                            scope.updateHeadImg = function () {
                                ...
                                uploadImgService.upLoadImg(template24_1Service.getConfigByAspectRatio(1), 1, scope.upLoadHeadImgFinish, '', { serviceType: "headerImg" });
