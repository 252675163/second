...
define(["ionic"], function () {
    angular.module("Template6_7.directives", [])
        .directive("template6by7", [
            "$window", "$timeout", "$rootScope", "template6_7Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope, template6_7Service, uploadImgService, promptBarService, maskService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template6_7/template.html",
                    link: function (scope, iElement, iAttr) {
                        ...
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template6_7Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template6_7Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template6_7Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
