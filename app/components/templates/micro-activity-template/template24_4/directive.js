...
define(["ionic"], function () {
    angular.module("Template24_4.directives", [])
        .directive("template24by4", [
                "$window", "$timeout", "$rootScope", "template24_4Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function ($window, $timeout, $rootScope, template24_4Service, uploadImgService, maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template24_4/template.html",
                        link: function (scope, iElement, iAttr) {
                            ...
                            scope.renderListByInit = function () {
                                if (scope.isView && ($rootScope.$state.params.step != "2")) {
                                    ...
                                    template24_4Service.getChristmasUserInfos(scope.userId, scope.userType, 1).success(function (result) {
                                        if (result.status == 1) {
                            ...
                            scope.loadMore = function () {
                                template24_4Service.getChristmasUserInfos(scope.userId, scope.userType, scope.page.currentIndex + 1).success(function (result) {
                            ...
                            template24_4Service.setRenderCallback(scope.renderListByInit);
                            init();
