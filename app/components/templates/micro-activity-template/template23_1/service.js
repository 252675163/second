...
define(['ionic', 'services/net/activity-template'], function () {
    return angular.module('Template23_1.Service', ['services.net.activityTemplate']).
        factory('template23_1Service', ['$http', 'activityFormService', 'activityTemplateService', function ($http, activityFormService, activityTemplateService) {

            var service = {};
            service.model =
            {
                title: '满天星教育集团',
                description: '一条鱼=一张学费抵扣券\n每人抵扣上限为300元\n养鱼数排行前10名可获得额外大奖',
                name: 'XXX',
            ...
            service.getGrassUrlByGrassCount = function (score) {
                ...
                defaultGifUrl = defaultGifUrl.replace("item1", "item" + score);
                switch (score) {
                    case 1:
                        defaultGifUrl = defaultGifUrl.replace("item1", "item" + score);
                        break;
            ...
            service.getGrassClasByGrassCount = function (score) {
                var url = "/template1_group1_gif.gif";
                score = score > 5 ? 5 : score;
                if (score == 0) {
                    return "";
                }
                else {
                    return url.replace("group1", "group" + score);
                }
                //switch (score) {
                //    case 0:
                //        return "";
