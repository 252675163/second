...
define(["ionic"], function() {
    angular.module("microOldNewTemplate2_2_7.directives", ["ionic"])
        .directive("microOldNewTemplate227", [
                "$window", "$timeout", "microOldNewTemplate2_2_7Service", "$ionicModal", "$rootScope", "$ionicScrollDelegate", "maskService", "promptBarService", "commonNetService", function($window, $timeout, microOldNewTemplate2_2_7Service, $ionicModal, $rootScope, $ionicScrollDelegate, maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template2_2_7/template2_2_7.html",
                        link: function(scope, iElement, iAttr) {
                            ...
                            function init() {
                                if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                    scope.sectionModel.templateModel = angular.copy(microOldNewTemplate2_2_7Service.model);
                            ...
                            scope.submitInfo = function() {
                                ...
                                scope.isSubmit = true;
                                microOldNewTemplate2_2_7Service.saveInfo(date, scope.userType).then(function(result) {
                                    scope.isSubmit = false;
                                    if (result.data.status == 1) {
                                        if (scope.userType == "old") {
                                            //分享遮罩
                                            maskService.showMask("", 0, false, 2).then(function() {
                            ...
                            //表单校验
                            function validForm() {
                                var validState = microOldNewTemplate2_2_7Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                                if (validState == 1) {
