...
define(["ionic"], function() {
    angular.module("Template19_4.directives", [])
        .directive("template19by4", [
                "$window", "$timeout", "$rootScope", "template19_4Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function($window, $timeout, $rootScope, template19_4Service, uploadImgService, maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template19_4/template.html",
                        link: function(scope, iElement, iAttr) {
                            ...
                            scope.renderListByInit = function() {
                                if (scope.isView && ($rootScope.$state.params.step != "2")) {
                                    ...
                                    template19_4Service.getChristmasUserInfos(scope.userId, scope.userType, 1).success(function (result) {
                                        if (result.status == 1) {
...
//                                            scope.page = result.data.page;
                            ...
                            scope.loadMore = function() {
                                template19_4Service.getChristmasUserInfos(scope.userId, scope.userType, scope.page.currentIndex + 1).success(function (result) {
                            ...
                            template19_4Service.setRenderCallback(scope.renderListByInit);
                            init();
                        }
