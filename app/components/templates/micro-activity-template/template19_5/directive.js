...
define(["ionic"], function() {
    angular.module("Template19_5.directives", [])
        .directive("template19by5", [
                "$window", "$timeout", "$rootScope", "template19_5Service",  "maskService", "promptBarService", "commonNetService", function($window, $timeout, $rootScope,template19_5Service,  maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template19_5/template.html",
                        link: function (scope, iElement, iAttr) {
                            ...
                            function init() {
                                ...
                                if (scope.isView && ($rootScope.$state.params.step != "2")) {
                                    ...
                                    template19_5Service.getGrassRanks(userId, scope.userType).success(function (result) {
                                        if (result.status == 1) {
                                            ...
                                            // warning 执行顺序 todo
                                            $timeout(function() {
                                                scope.userInfo = template19_5Service.getUserInfo();
                                            });
                            ...
                            template19_5Service.setRenderCallback(init);
