...
define(["ionic"], function () {
    angular.module("Template3_7.directives", [])
        .directive("template3by7", [
            "$window", "$timeout", "$rootScope", "template3_7Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function ($window, $timeout, $rootScope, template3_7Service, uploadImgService, maskService, promptBarService, commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template3_7/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template3_7Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template3_7Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template3_7Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
                        ...
                        scope.updateImg = function(imgIndex){
                            ...
                            uploadImgService.upLoadImg( template3_7Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]),1,scope.upLoadFinish);
                        };
