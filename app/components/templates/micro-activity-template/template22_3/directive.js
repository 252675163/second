...
define(["ionic"], function () {
    angular.module("Template22_3.directives", [])
        .directive("template22by3", [
            "$window", "$timeout","$rootScope", "template22_3Service", "uploadImgService", "promptBarService", "maskService", "commonNetService", function ($window, $timeout, $rootScope,template22_3Service, uploadImgService, promptBarService, maskService,commonNetService) {
                return {
                    restrict: 'EA',
                    templateUrl: "components/templates/micro-activity-template/template22_3/template.html",
                    link: function (scope, iElement, iAttr) {

                        //status表明当前模板的状态：view（查看），preview（预览），edit（编辑）
                        function init(){
                            if (angular.equals(scope.templateModel, {}) || angular.isUndefined(scope.templateModel)) {
                                scope.sectionModel.templateModel = angular.copy(template22_3Service.model);
                        ...
                        scope.submitInfo = function () {
                            ...
                            scope.isSubmit = true;
                            template22_3Service.saveInfo(date, scope.userType).then(function (result) {
                        ...
                        //表单校验
                        function validForm() {
                            var validState = template22_3Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                            if (validState == 1) {
                        ...
                        scope.updateImg = function (imgIndex) {
                            ...
                            uploadImgService.upLoadImg(template22_3Service.getConfigByAspectRatio(scope.imgAspectRatio[imgIndex]), 1, scope.upLoadFinish);
                        };
