...
define(["ionic"], function() {
    angular.module("Template14_5.directives", [])
        .directive("template14by5", [
                "$window", "$timeout", "$rootScope", "template14_5Service",  "maskService", "promptBarService", "commonNetService", function($window, $timeout, $rootScope,template14_5Service,  maskService, promptBarService, commonNetService) {
                    return {
                        restrict: "EA",
                        templateUrl: "components/templates/micro-activity-template/template14_5/template.html",
                        link: function (scope, iElement, iAttr) {
                            ...
                            function init() {
                                ...
                                if (scope.isView && ($rootScope.$state.params.step != "2")) {
                                    ...
                                    template14_5Service.getGrassRanks(userId, scope.userType).success(function (result) {
                                        if (result.status == 1) {
                                            ...
                                            $timeout(function() {
                                                scope.userInfo = template14_5Service.getUserInfo();
                                            });
                            ...
                            template14_5Service.setRenderCallback(init);
