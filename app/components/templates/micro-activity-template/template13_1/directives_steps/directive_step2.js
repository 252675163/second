...
define(["ionic", "components/templates/micro-activity-template/template13_1/service"], function() {
    angular.module("Template13_1Step2.directives", ["Template13_1.Service"])
        .directive("template13by1Step2", [
                "$window", "$timeout", "$rootScope", "template13_1Service", "uploadImgService", "maskService", "promptBarService", "commonNetService", function($window, $timeout, $rootScope, template13_1Service, uploadImgService, maskService, promptBarService, commonNetService) {
                    return {
                        ...
                        templateUrl: "components/templates/micro-activity-template/template13_1/templates_steps/template_step2.html",
                        link: function(scope, iElement, iAttr) {
                            ...
                            function init() {
                                if (scope.status != "view") {
                                    ...
                                    template13_1Service.getWeixinUserInfo().success(function (result) {
                                        if (result.status == 1) {
                                            scope.userInfo = {
                            ...
                            scope.submitInfo = function() {
                                ...
                                scope.isSubmit = true;
                                template13_1Service.saveInfo(date).then(function(result) {
...
//                                            if (config) {
//                                                config.link = link;
//                                                config.title = scope.userInfo.name ? scope.userInfo.name + "喊你帮TA种草啦~" : config.title;
//                                            }
...
//                                            });
                            ...
                            //表单校验
                            function validForm() {
                                var validState = template13_1Service.isValid(scope.userInfo.name, scope.userInfo.phone);
                                if (validState == 1) {
                            ...
                            scope.updateHeadImg = function() {
                                ...
                                uploadImgService.upLoadImg(template13_1Service.getConfigByAspectRatio(1), 1, scope.upLoadHeadImgFinish, '', { serviceType: "headerImg"});
